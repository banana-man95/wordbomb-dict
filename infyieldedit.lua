...
CMDs[#CMDs + 1] = {NAME = 'orbit [player]', DESC = 'Orbits a player, duhhhh'}
CMDs[#CMDs + 1] = {NAME = 'unorbit', DESC = 'Stops orbiting a player, duhhhh'}
CMDs[#CMDs + 1] = {NAME = 'iyorbit [player] [speed] [distance]', DESC = 'the bad iy one: Makes your character orbit around a player with an optional speed and an optional distance'}
CMDs[#CMDs + 1] = {NAME = 'uniyorbit', DESC = 'Disables bad infinite yield orbit'}
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/banana-man95/wordbomb-dict/main/infyieldedit.lua'))()")
	end
end)
...
addcmd("orbit", {}, function(args, speaker)
    ...
    if players and #players > 0 then
        local target = players[1] -- Take the first player from the list
        local targetPlayer = Players[target]
        
        if targetPlayer and targetPlayer.Character then
            notify("-", 'Now orbiting: ' .. targetPlayer.Name)
            local P = Instance.new("Part")
            P.Parent = targetPlayer.Character
            P.Transparency = 1
            P.Name = "ThePart"
            ...
            W.Parent = P
            W.Part1 = targetPlayer.Character.HumanoidRootPart
            W.Part0 = P
            ...
            local sine, spin, spin2 = 0, 0, 0
            local change = 1
            
            local connection
            connection = game:GetService("RunService").RenderStepped:Connect(function()
                if not (speaker.Character and speaker.Character:FindFirstChild("HumanoidRootPart") and
            ...
            notify("Error", 'Target player character not found.')
        end
    else
...
addcmd("iyorbit", {}, function(args, speaker)
...
addcmd("uniyorbit", {}, function(args, speaker)
...
local CollisionGroup = game:GetService("PhysicsService"):CreateCollisionGroup("UnanchoredObjects")
...
addcmd('tpunanchored',{'tpua'},function(args, speaker)
        ...
        for _,part in pairs(workspace:GetDescendants()) do
            if Players[v].Character:FindFirstChild('Head') and 
               ...
               part.Name ~= "HumanoidRootPart" then
                ...
                part.CollisionGroup = "UnanchoredObjects"
    ...
    for _, part in ipairs(unanchoredParts) do
        PhysicsService:SetPartCollisionGroup(part, "UnanchoredObjects")
    end

    PhysicsService:CollisionGroupSetCollidable("UnanchoredObjects", "Default", false)
    PhysicsService:CollisionGroupSetCollidable("UnanchoredObjects", "UnanchoredObjects", true)
end)
